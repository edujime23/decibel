...
pub async fn run_ipc_loop(shm_path: String, app_state: Arc<AppState>, tx_cmd: Sender<AudioCommand>) {
    ...
    let file = OpenOptions::new()
        ...
        .open(shm_file_path).expect("Failed to open Shared Memory file");
    ...
    tokio::spawn(async move {
        #[cfg(windows)]
        {
            ...
            let server = ServerOptions::new().first_pipe_instance(true).create(pipe_name).expect("Failed to create Windows Named Pipe");
    ...
    loop {
        ...
        for idx in 0..16 {
            category_volumes[idx] = unsafe { std::ptr::read_volatile(mmap.as_ptr().add(48 + idx * 4) as *const f32) };
        }
        ...
        if ver == ver_check {
            if listener_pos != last_pos || listener_fwd != last_fwd || listener_up != last_up || category_volumes != last_category_volumes || engine_flags != last_engine_flags {
                let _ = tx_cmd.send(AudioCommand::UpdateListener {
                    pos: listener_pos, fwd: listener_fwd, up: listener_up, category_volumes, engine_flags,
                });
                last_pos = listener_pos; last_fwd = listener_fwd; last_up = listener_up;
                last_category_volumes = category_volumes; last_engine_flags = engine_flags;
            }
        }
        ...
        if voxel_version != local_voxel_version {
            ...
            tokio::task::spawn_blocking(move || {
                crate::phonon::rebuild_acoustic_mesh(app_state_task.scene, app_state_task.context, &*voxel_bytes, [cx, cy, cz]);
            });
        }
        ...
        while local_read_seq < java_write_seq {
            ...
            if opcode == 0 {
                ...
                let _ = tx_cmd.send(AudioCommand::PlaySound { uid, pos: [x, y, z], volume, pitch, asset_hash, is_relative, is_spatial, category_id });
            } else if opcode == 1 {
                let uid = unsafe { read_volatile_u32(&mmap, offset + 4) };
